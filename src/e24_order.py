...
ca,sea,cova=lsq(X,np.abs(epar))
cc,sec,covc=lsq(X,np.abs(eperp))
print("\n                      along-track        cross-track        ANISOTROPIC (along-cross)")
...
json.dump(dict(n=int(len(A)),aniso_accel=float(ca[3]-cc[3]),aniso_speed=float(ca[1]-cc[1]),
          ...
          open('/work/experiments/e24_order/result.json','w'),indent=1)
print("WROTE")
