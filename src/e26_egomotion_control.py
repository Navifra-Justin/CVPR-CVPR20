...
SEQS=['zurich_city_00_a','zurich_city_09_a','zurich_city_10_a','zurich_city_02_a']
MIN_EV=400
...
for SEQ in SEQS:
    ...
    for a,b in exp:
        ...
        for r in by_t[key]:
            ...
            for mask,acc in ((inb,cb),(core,cc),(outer,cr)):
                n=int(mask.sum())
                if n<MIN_EV: continue
                gx,gy=grad(xx[mask].astype(float),yy[mask].astype(float),ts[mask])
                g=np.hypot(gx,gy)
                if g>1e-12: acc.append((gx*u[0]+gy*u[1])/g)
    f.close()
    def rep(v):
        v=np.array(v); return (round(float(v.mean()),4), len(v),
                               round(float(abs(v.mean())*np.sqrt(len(v))),1)) if len(v) else (None,0,0)
    B,C,R=rep(cb),rep(cc),rep(cr)
    out[SEQ]=dict(box=B,core=C,ring=R)
    print(f"{SEQ:<20} BOX {B[0]:+.4f} (n={B[1]}, {B[2]:.1f}SE)   "
          f"CORE {C[0]:+.4f} (n={C[1]}, {C[2]:.1f}SE)   RING {R[0]:+.4f} (n={R[1]}, {R[2]:.1f}SE)",flush=True)
print("\nverdict: CORE > BOX > RING means the object carries it; RING ~ BOX means ego-motion.")
os.makedirs('/work/experiments/e26_egomotion',exist_ok=True)
json.dump(out,open('/work/experiments/e26_egomotion/result.json','w'),indent=1)
print("WROTE")
