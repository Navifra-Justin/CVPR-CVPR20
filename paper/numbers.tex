...
\newcommand{\dsecCotriggerPct}{100.00}
...
%% No detector, no checkpoint, no velocity denominator, no assumed centre noise.
...
\newcommand{\nthirdDiff}{629\,442}
%% E10 mains flicker
\newcommand{\flickerLineHz}{100.00}   %% Hz, strongest night line
...
%% --- E09, the per-track structure of the frame-centred residual ------------
...
\newcommand{\labelSpeedMedian}{40}         % px/s, median box-centre speed
...
%% --- E13, the label centre noise, solved rather than bounded ---------------
...
\newcommand{\sigmaCtotal}{0.4974}          % px, total centre noise
...
%% difference, which shares the label-centre noise with the residual and manufactures
...
\newcommand{\outRowsTotal}{35\,523}        %% E27, matches before requiring both neighbours
...
%% injection.log arm 3). A forward difference shares the label-centre noise with the
...
%% ===== E28 measured 2026-09-07 (experiments/e28_gen1_label_noise): the label-centre
...
\newcommand{\labelTracksGen}{691}          %% E28, tracks of >= 12 labels on a uniform grid
...
%% Z = N|C|^2 at 100 Hz, with an off-frequency arm at 137 Hz. Z is Exp(1) under
%% uniform phase, so its null median is ln 2 = 0.693 whatever the sequence.
...
\newcommand{\eivVarNoise}{0.6}             %% px^2/s^2, 2 sigma^2 / (dt_f+dt_b)^2
\newcommand{\eivLambda}{0.995}             %% the attenuation factor on these rows
\newcommand{\eivShift}{0.1}                %% ms, how far the correction moves tau
...
%% the random-phase floor of the modulation depth depends on the event count, so the
%% six-sequence table (400 events) does not share the floor of E19 (200 events)
