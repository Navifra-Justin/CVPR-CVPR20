...
while true; do
  ...
  if [ "$f0" -ge 10000 ] || [ "$f1" -ge 10000 ]; then
    ...
    echo "$(date +%H:%M:%S) launching on GPU$G"
    docker run --rm --gpus "\"device=$G\"" --memory=32g --user $(id -u):$(id -g) -e HOME=/tmp \
      -e DEV=cuda:0 -e NSEQ=12 -e NSAMP=40 -e KLAG=19 -e OUTJ=/work/experiments/e42_recurrent_support/result_k19.json -v $PWD:/work -w /work \
      cvpr19-gpu-g1:torch2.7.1-cu128 \
      bash -lc "pip install -q --user hdf5plugin omegaconf hydra-core einops StrEnum 2>&1|tail -0; python3 -u /work/src/e42_recurrent_support.py"
